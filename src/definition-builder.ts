...
export class DefinitionBuilder {
  ...
  private applyParameters(field: DefinitionField, mappedParameters: DatumParameters): DefinitionConstr {
    ...
    if ('list' in field) {
      field.list = (field.list as DefinitionField[])?.map((fieldParameter: DefinitionField) => {
        return this.applyParameters(fieldParameter, mappedParameters);
      });
    }
      ...
      (definedDefinition.list as DefinitionField[])
        .map((fieldParameter: DefinitionField, index: number) => {
          ...
          if (templateDefinition.list as DefinitionField[]) {
            return this.extractParameters(fieldParameter, (templateDefinition.list as DefinitionField[])[index], foundParameters);
          } else {
