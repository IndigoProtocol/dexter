...
export class SwapRequest {
  ...
  public submit(): DexTransaction {
    ...
    this._dexter.availableDexs[this._liquidityPool.dex].buildSwapOrder(this._liquidityPool, defaultSwapParameters, this._withUtxos).then((payToAddresses: PayToAddress[]) => {
  ...
  public async submitToDexs(liquidityPools: LiquidityPool[]): Promise<DexTransaction> {
    if (!this._dexter.walletProvider) {
      throw new Error('Wallet provider must be set before submitting a swap order.');
    }
    if (!this._dexter.walletProvider.isWalletLoaded) {
    ...
    const payToAddresses = liquidityPools.map((pool) => {
      if (!this._swapInToken) {
        throw new Error('Swap in token must be set before submitting a swap order.');
      }
      if (this._swapInAmount <= 0n) {
      ...
      const datum = this.getDefaultDatumForLiquidityPool(pool);
      const payToAddresses = this._dexter.availableDexs[pool.dex].buildSwapOrder(pool, datum, this._withUtxos);
      return payToAddresses;
    });

    for await (let payToAddress of payToAddresses) {
      this.sendSwapOrder(swapTransaction, payToAddress);
    }

    return swapTransaction;
  }

  private getDefaultDatumForLiquidityPool = (liquidityPool: LiquidityPool) => {
    const datumParameters: DatumParameters = {
      [DatumParameterKey.SenderPubKeyHash]: this._dexter.walletProvider?.publicKeyHash(),
      [DatumParameterKey.SenderStakingKeyHash]: this._dexter.walletProvider?.stakingKeyHash(),
      [DatumParameterKey.ReceiverPubKeyHash]: this._dexter.walletProvider?.publicKeyHash(),
      [DatumParameterKey.ReceiverStakingKeyHash]: this._dexter.walletProvider?.stakingKeyHash(),
      [DatumParameterKey.PoolIdentifier]: liquidityPool.identifier,
      [DatumParameterKey.SwapInAmount]: this._swapInAmount,
      [DatumParameterKey.MinReceive]: this.getMinimumReceive(),
      [DatumParameterKey.SwapInTokenPolicyId]: this._swapInToken === 'lovelace' ? '' : (liquidityPool.assetA as Asset).policyId,
      [DatumParameterKey.SwapInTokenAssetName]: this._swapInToken === 'lovelace' ? '' : (liquidityPool.assetA as Asset).nameHex,
      [DatumParameterKey.SwapOutTokenPolicyId]: this._swapOutToken === 'lovelace' ? '' : (liquidityPool.assetB as Asset).policyId,
      [DatumParameterKey.SwapOutTokenAssetName]: this._swapOutToken === 'lovelace' ? '' : (liquidityPool.assetB as Asset).nameHex,
    };
